...
MIG: dict[tuple[str, str], dict] = {}        # (kosis 시군구코드 5자리 or 시도 2자리, ym) → {"in":총전입,"out":총전출,"net":순이동}
...
def load_stations(conn) -> list[tuple[float, float, str | None, str | None]]:
    rows = conn.execute("SELECT s.lat, s.lon, s.opened_ym, s.status_date, s.status, p.destination_tier, p.name AS pname, s.name AS sname, p.line_id FROM transit_station s "
