...
### 2026-09-07c — 이중 희소성 가설(좋은 입지 × 노후) 부분 채택
- 규칙 단독: 급지 상위(≤2) & 25년 이상은 진입 2008~2021 전 14개 연도에서 상위10% 적중 **0.442**(입지 단독 0.232, 무작위 0.10), 미사용 3년 구간 0.625. 좋은 입지의 신축은 0.185 로 입지 단독보다 낮고, 낮은 급지의 노후는 −2.5%p(미사용 구간 적중 0.000) → **노후는 좋은 입지와 결합할 때만 값이 된다**(연구로그 §35).
- 조건: 상승 국면 한정(2008~2011 진입 −9~−14%p), 급지 상위 한정, 연 4~18개 소표본. 순위 모델 변수로는 개선 0(S3 = S0) → **운영 v0.8 미변경, 조건부 선별 규칙으로만 등록**.
- 전제 확인: 준신축(6~15년)/구축 프리미엄이 2007년 1.02 → 2026년 1.28 로 구조적 상승(신축 선호 강화 지지). 신축(≤5년) 프리미엄은 2017년 1.55 정점 후 1.23 으로 축소.

### 2026-09-07b — 세 독립 실험과 사회적 승강장 H1 (운영 변경 없음)
- 사전등록 규격 2건(`EXPERIMENT_PROTOCOL_20260907.md`, `SOCIAL_ESCALATOR_SPEC_v0.1.md`)에 따라 규칙·가중치를 실행 전에 고정하고 검증. **네 가설 모두 기준모델 v0.8 을 넘지 못해 운영 반영 없음.**
- 판정: 구매력 성장 **검증대기**(소득 성장만, 미사용 3년 구간에서 세 지표 + 이나 연도 2개 · 국민연금 스냅샷 5개 한계), 조건부 저평가 **폐기**(정상가격 R² 0.87 이나 수익 예측 개선 0), 대체재 경쟁 **폐기**, 사회적 승강장 H1 **폐기**(단순 총유입 대조모델과 동일 = 재표현). 연구로그 §33·§34.
- 방법론 기록: 자료 있는 표본만 비교하면 기준모델 성적이 0.417→0.60 으로 보이는데 이는 전부 연도(표본) 효과였다. 이후 모든 실험은 **같은 표본에서 기준모델 재평가**를 의무로 한다.
- 신규 상시 자료: 시군구 출발지→도착지 이동(2012~2025, 25만 행) 등급 B. H2(생애주기)는 연령 축 부재로 BLOCKED_DATA, H3(관계망)는 자료 없어 미착수.

### 2026-09-07a — '종인 이론'(동아1단지 사고 과정) 백테스트
- 사고 과정을 변수 7개·규칙 4개로 옮겨 검증(연구로그 §32). 규칙 단독: 재건축 레이더(30년↑·용적률<200·1,000세대↑) 상위10% 적중 24~26%(무작위 10%)·5년 상대수익 +5.5~7.7%p, 1급 역 선점 +5.8~6.6%p; '시도 대비 저가·접근'과 '타지역 유입'은 정보 없음. 변수로 E 에 더하면 개선 없음(재건축 레이더 ±0, 밀려나는수요 −7.5pp).
- 결정: 순위 모델 v0.8 유지. 재건축 레이더·1급 역 선점은 E 상위권 안에서 고르는 **보조 규칙**(§14·§30.6 관측 지표)으로만 쓴다.

### 2026-09-06e — 교통 선점 Bull 반영(표본 30건 미만 진행 결정)·정비 단계 사건연구·동아1단지 논리 대조
- 종인님 결정: 표본 수와 무관하게 진행. `invest/exit_price.TransitInput`: 착공 상태 1급 노선 역 1km 안 단지(30단지)의 **Bull 시나리오에만** +5.1%(완행)/+8.6%(급행) PROXY 반영, Base·Bear 미반영(개통 뒤 되돌림). 개통 시 자동 소멸(연구로그 §30.6).
- §14 보강: 정비 단계 도달 사건연구(§31.1) — 조합설립 도달 시점까지 5년간 자기 가격 +56%(시군구 대비 +7.1%p), 인천 +30%(+12.4%p), 도달 뒤 2년은 시군구와 동행. 정비 프리미엄은 조합설립까지 집중.
- 동아1단지: 종인님 사고 과정 7단계를 데이터로 대조(§31). 지지 ①GTX-B 선점·④물리 요건, 불지지 ②부평구 순유출·⑥학군 평균, 미확인 ③상권·⑤캠프마켓. 목표 7.6억은 인천 조합설립 사례 최대치·현재 신축 상한 밖; 기본 기대 6.0억(조합설립 성공 시), 상위 7.0~7.2억. 매도 시점은 조합설립 또는 GTX-B 개통 6개월 전 중 먼저 오는 때.

### 2026-09-06d — 교통 선점효과 검증 완료(사건연구 채택·순위 변수 미채택)
- 연혁표(예타·기본계획·착공·개통 24개 사업)·급행 정차역표로 과거 계획역을 복원해 검증. 사건연구(연구로그 §30.3): 1급 노선 역 1km 안 단지는 **착공~개통 사이** 시군구 대조군보다 +5.1%p(14건 중 12건), 급행 정차역 +8.6%p, 2·3급 노선 효과 없음, 개통 뒤 24개월 되돌림 → §12 '개통 자체 가점 0' 유지하되 **"착공된 1급 노선 역 1km & 미개통"을 단지 판단 단계 관측 지표로 등록**(앱 표기).
- 순위 모델: 5년(§30.4)·2년(§30.5) 지평 모두 착공 복원·급행 변수로 포착률 개선 없음(±1pp) → v0.8 유지. 표본 30건 초과 시 재검증.

### 2026-09-06c — 교통 선점효과를 가격 이론에 대입(§12 실험)
- 목적지 등급 기반 교통 변수 6개(1·2급 역거리·접근점수·계획역·최근 개통 선점) 추가(`panel.TRANSIT_GROUPS`). 횡단면: 급지·1급지 거리·학원·직장이 가격의 82~87%를 설명하고 교통은 +1~3%p 만 더함(교통 가치는 급지에 이미 반영, 2012→2021 감소). walk-forward: 확인 구간 +1.8~2.2pp / 선택 구간 −2.6pp → 채택 보류(연구로그 §30). 계획역 공표 연혁 자료 확보 뒤 재검증.
- 앱 '앞으로' 탭에 가장 가까운 1·2급 노선 역과 1.5km 안 계획 1급 역(48단지, GTX-B 등)을 표기(점수 미반영).

### 2026-09-06b — 청약경쟁률·건축물대장 수집, 교통 목적지 등급
- 청약홈 경쟁률 5.5만 건 → 시군구·월 731행. 직전 6개월 경쟁률과 다음 12개월 시군구 상대수익 Spearman +0.21(n=482), 지난 12개월과는 −0.02 → 약한 **선행** 신호(5:1 이상 +3.5~4.6%p, 미달 −1.4%p). 5년 walk-forward 는 불가(2020~). 앱 '앞으로' 탭에 시군구 최근 12개월 경쟁률 표기(연구로그 §28.1).
- 건축물대장 총괄표제부 `vlRatEstmTotArea` 로 §14 `existing_far` 755단지 VERIFIED(동아1단지 181.7%). 직전 추정치(전용×1.15)는 중앙 13% 과소였음(§28.2).
- 교통 호재 규칙(`RULE_TRANSIT_BY_DESTINATION_20260906.md`) 1차 반영: 노선 목적지 등급표 + DB 태깅. 등급별 개통 효과 재집계 결과 어느 등급도 개통 자체 상승 없음(§29) → §12 '이름 가점 0' 유지. analogue 등급 내 매칭·UNKNOWN 무가점은 코드 반영 예정.
