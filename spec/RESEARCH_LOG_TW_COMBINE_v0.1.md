...
`reports/top100_before_combine_2026-09-04.json` vs `reports/top100_latest.json` (3억 · 5년 · 같은 as_of).

- TOP10 유지 5/10 · TOP20 유지 13/20 · 새로 TOP100 진입 33개(부영1단지 5위, 용인 구성역 효성해링턴 23위, 송내주공1단지 25위, 청천쌍용 32위, 부평 욱일 42위, 부평 경남1차 45위 …).
- 변경 전 TOP20 의 driver 는 전부 `relative` 를 포함했다 → 이 모델이 상위권을 만든 주요 축이었음이 확인된다. 제거 후 driver 는 value·risk·supply·jeonse·access 로 재편.
...
변경 후 TOP10: 용인보라해링턴 · 문촌마을16단지 · 안산 라프리모 · 안산중흥S-클래스더퍼스트 · 부영1단지 · 동두천 에코휴먼빌2차 · 논현파크포레 · 동두천동양엔파트 · 송산푸르지오 · 호반베르디움 더레이크3차.
이 점수 순위는 여전히 discovery 용이며(§13 TW 미완성), 순위 안정성 지표(평균순위·TOP10 생존율·P90)는 다음 라운드에서 다시 낸다.
...
## 6. 테스트
전체 테스트 스위트: 가중치 변경(relative·redevelopment → 0) 후 재실행 **통과(exit 0)**, 2026-09-04.
