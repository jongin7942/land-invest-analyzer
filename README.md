...
- 진행 상황: **PHASE 0~7 + 3.9 완료** (기반·수집·대표가격·호가·규제세금대출·상대가치·촉매·재건축·실투자금/ROE·현금흐름/세후 IRR) · PHASE 8(투자점수·랭킹·Why Not) 착수 예정
...
## PHASE 7 — 현금흐름 · Peak Equity · 세후 IRR · Stress Test

PHASE 3.9 까지가 "한 시점의 값"이었다면 여기서 그걸 **시간축에 올린다.**

```bash
python -m apt_engine.cli cashflow "동아1단지" \
    --price 6 --sale 8 --holding 5 \
    --income 1 --rate 0.045 --holding-cost 150 \
    --occupancy 실거주 --stress --save
```

### Initial Equity 와 Peak Equity 는 다른 값이다

매수 시점의 실투자금만 보면 **보유하는 동안 더 들어가는 돈**을 놓친다.
역마진이면(월 현금흐름이 마이너스면) 실제로 묶이는 돈은 처음보다 커진다.

```
    Initial Equity  1.33억
    Peak Equity     2.55억   ← 보유 중 가장 많이 묶인 내 돈
```

`1억당 이익` 도 Initial 이 아니라 **Peak Equity 로 나눈다.** 실제로 묶인 돈이
분모여야 "내 돈이 얼마나 일했나" 를 제대로 말할 수 있다.

### 총 수익률이 아니라 세후 IRR

같은 5천만원도 2년에 벌면 좋은 투자고 10년 걸리면 예금만 못하다.
IRR 은 미분 없이 **이분법**으로 푼다 — 부동산 현금흐름은 부호가 여러 번 바뀔 수
있어 뉴턴법이 발산할 때가 있고, 그때 이상한 값을 내놓느니 "구할 수 없다"고 말한다.

### 거주 형태마다 현금흐름이 다르다

| | 임대수입 | 보증금 |
|---|---|---|
| `실거주` | 없음. 아끼는 주거비는 **추정하지 않고** `--imputed-rent` 로 받는다 | — |
| `임대` | 월세 | t=0 차감 · t=n 반환 |
| `전세승계` | 없음 | t=0 차감 · t=n 반환 |

상환방식도 나눈다(원리금균등 / 원금균등 / 만기일시). "이자 = 원금 × 금리 × 년수"로
뭉뚱그리면 원리금균등에서 이자를 과대계상하고 만기일시에서 원금 상환 시점을 놓친다.
**스트레스 금리는 여기 쓰지 않는다** — 그건 DSR 한도용이고, 실제 이자는 계약 금리다.

### 모르면 IRR 을 내지 않는다

보유세 과세표준은 **공시가격**에서 나온다. "시세의 몇 %" 로 추정하면 그게 곧
지어낸 숫자라, `--official-price` 를 받고 없으면 보유세가 확인 불가다.
그러면 그 해 순현금흐름도, IRR 도 만들지 않는다. 급하면 `--holding-cost` 로
연간 보유비용을 직접 넣을 수 있다.

### 시나리오와 Stress Test

Bear/Base/Bull 세 구간을 내고, **위험조정 기대수익**(Base 이익 ÷ Bear 손실)까지
계산한다. Bear 에서도 이익이면 관측된 위험이 없으므로 그 비율은 내지 않는다.

Stress Test 는 **한 번에 하나씩만** 흔든다 — 금리 +2%p / 매도가 −20% /
보유기간 +3년 / 공실. 두 개를 같이 흔들면 어느 쪽이 원인인지 알 수 없다.
배율과 충격 크기는 관측된 분포가 아니라 **가정**이고, 결과에 그렇게 표시된다.
