"""교통 목적지 등급·선점 변수(TRANSIT_GROUPS) 검증 — 종인님 2026-09-06 "교통 선점효과를 이론에 대입" — §24 와 같은 절차 (2026-09-06).
패널을 새로 만들어(logs/_exit_panel_transit.pkl) E vs E+그룹(같은 표본 ridge), 부스팅(E+POLICY, 전체행)을 비교한다.
...
def main() -> int:
    ...
    E = et.E; G = panel_mod.TRANSIT_GROUPS; P = panel_mod.FEATURE_SETS["T_transit"]
    out = {"coverage": {f: round(sum(1 for r in rows if r.x.get(f) is not None) / len(rows), 3) for f in panel_mod.TRANSIT}}
    ...
    r = same_sample(E, E + panel_mod.TRANSIT); out["ridge_same_sample"]["교통전부"] = r
    et.log(f"ridge 같은표본 E vs E+교통전부: {r['mean_E']} → {r['mean_E+']} ({r['wins']}/{r['n_years']} 우세)")
